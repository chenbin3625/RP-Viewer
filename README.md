...
RP-Viewer 是一个自托管原型评审平台，可自动扫描 Axure、墨刀等工具导出的原型目录，提供卡片式浏览、iframe 预览和类 Figma 的定点评论能力。

## Screenshots / 界面截图

**Home - Card-based browsing / 首页 - 卡片式浏览**

![Home](docs/images/home.png)

**Preview - iframe embedded rendering / 原型预览 - iframe 内嵌加载**

![Preview](docs/images/preview.png)

**Comments - Click to pin / 评论 - 点击定位**
...
### 2. Prepare Prototype Files / 准备原型文件
...
```
prototypes/
├── ProjectA/                  ← Category folder / 分类文件夹
│   ├── README.md              ← Optional: title + description / 可选：标题与描述
│   ├── icon.png               ← Optional: custom icon / 可选：自定义图标
│   ├── Homepage/              ← Prototype folder (contains index.html) / 原型文件夹（含 index.html）
│   │   ├── index.html
│   │   └── ...
│   └── Admin/
│       ├── index.html
...
```yaml
# Root directory for prototype files / 原型文件根目录
...
| Source / 来源 | `prototype_dir` | `port` |
|--------|-----------------|--------|
| Default / 默认 | `./prototypes` | `8080` |
| Config file / 配置文件 | `prototype_dir` | `port` |
| Environment variable / 环境变量 | `PROTOTYPE_DIR` | `PORT` |

Flags / 命令行参数：

```bash
./proto-viewer -config /path/to/config.yaml   # Use a custom config file / 指定自定义配置文件
./proto-viewer -dev                            # Dev mode (proxies frontend to Vite) / 开发模式（前端代理到 Vite）
```
...
```bash
make build        # Production build (frontend + Go binary) / 构建生产版本（前端 + Go 二进制）
make dev          # Dev mode (frontend & backend hot reload) / 开发模式（前后端热更新）
make clean        # Clean build artifacts / 清理构建产物
```

## Tech Stack / 技术栈

- **Backend / 后端**: Go (net/http, embed, gzip)
- **Frontend / 前端**: React + TypeScript + Ant Design + React Router
- **Deployment / 部署**: Single binary with embedded frontend assets / 单二进制文件，前端资源内嵌
